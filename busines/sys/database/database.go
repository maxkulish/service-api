...
func StatusCheck(ctx context.Context, db *sqlx.DB) error {
	...
	for attempts := 1; ; attempts++ {
		...
		// Break out of the loop when context is done or timeout
	...
	// Make sure we didn't timeout or be concelled
	if ctx.Err() != nil {
		return ctx.Err()
	}

	// Run a simple query to determine connectivity. Running this query
	// forces a round trip through the database.
	const q = "SELECT true"
	var tmp bool
...
func WithinTran(ctx context.Context, log *zap.SugaredLogger, db *sqlx.DB, fn func(*sqlx.Tx) error) error {
	...
	// We can defer the rollbach since the code checks if the
...
// ExecContext is a helper function to execute
// a CUD operation with logging and tracing
...
func NamedExecContext(ctx context.Context, log *zap.SugaredLogger, db sqlx.ExtContext, query string, data any) error {
	...
	log.WithOptions(zap.AddCallerSkip(callerSkip)).Infow("database.NamedExecContext", "trace_id", web.GetTraceID(ctx), "query", q)
...
func namedQuerySlice[T any](ctx context.Context, log *zap.SugaredLogger, db sqlx.ExtContext, query string, data any, dest *[]T, withIn bool) error {
	...
	log.WithOptions(zap.AddCallerSkip(callerSkip)).Infow("database.NamedQuerySlice", "trace_id", web.GetTraceID(ctx), "query", q)
