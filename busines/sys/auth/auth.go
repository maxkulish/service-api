...
// NewAuth creates a new Auth to support authentication/authorization.
func NewAuth(activeKID string, keyLookup KeyLookup) (*Auth, error) {

	_, err := keyLookup.PrivateKey(activeKID)
	if err != nil {
		return nil, errors.New("activate KID does not exist in store")
	}
	...
	keyFunc := func(t *jwt.Token) (any, error) {
		...
		return keyLookup.PublicKey(kindID)
	}
