...
// NewStore constructs a user store for user api access.
...
func (s Store) Create(ctx context.Context, nu NewUser, now time.Time) (User, error) {
	if err := validate.Check(nu); err != nil {
		return User{}, fmt.Errorf("validating new user: %w", err)
	}
...
func (s Store) Update(ctx context.Context, claims auth.Claims, userID string, uu UpdateUser, now time.Time) error {
	...
	if err := validate.Check(uu); err != nil {
		return fmt.Errorf("validating updates: %w", err)
	}
...
// On success it returns a Claims User representing this user. The claims
// can be used to generate a token for future authentication.
func (s Store) Authenticate(ctx context.Context, now time.Time, email, password string) (auth.Claims, error) {
	...
	// Compare the provided password with the one we have stored. Use the
	// bcrypt comparison function to prevent timing attacks.
